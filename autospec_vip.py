...
class autospec_vip(iouonegirlPlugin):
    def __init__(self):
    ...
    def help_get_last(self):
        ...
        picked_player = None
        for player in bigger_team:
            if player.vip  is not True:
                picked_player = player
                minqlx.console_command("echo Autospec: Picking someone to spec based on VIP: {}".format(player.name))
                break

        if picked_player is None:
            picked_player = bigger_team[0]
            minqlx.console_command("echo Autospec: Picking someone to spec (but all VIPs in team(: {}".format(picked_player.name))

        minqlx.console_command("echo Autospec: Picked {} from the {} team.".format(picked_player.name, picked_player.team))
